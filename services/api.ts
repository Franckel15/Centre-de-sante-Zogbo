...
export const api = {
  ...
  settings: {
    get: async (key: string): Promise<string | null> => {
      try {
        const { data, error } = await supabase.from('site_images').select('url').eq('key', `setting_${key}`).maybeSingle();
        if (error || !data) {
          if (typeof window !== 'undefined' && window.localStorage) {
            return window.localStorage.getItem(`app_setting_${key}`);
          }
          return null;
        }
    ...
    set: async (key: string, value: string): Promise<void> => {
      try {
        if (typeof window !== 'undefined' && window.localStorage) {
          window.localStorage.setItem(`app_setting_${key}`, value);
        }
        await supabase.from('site_images').upsert({ key: `setting_${key}`, url: value });
      } catch (e) {
        console.warn(`Erreur sauvegarde setting_${key}:`, e);
      }
    }
  },
  ...
  blog: {
    ensureInitialized: async (): Promise<void> => {
      try {
        const isInit = await api.settings.get('blog_initialized');
        if (isInit === 'true') return;

        // Check if posts table already has records
        const { data: existing, error } = await supabase.from('posts').select('id').limit(1);
        if (!error && existing && existing.length > 0) {
          await api.settings.set('blog_initialized', 'true');
          return;
        }

        // Table has 0 rows and not initialized: seed default posts excluding any deleted
        const deletedIdsStr = await api.settings.get('deleted_blog_ids');
        const deletedIds: number[] = deletedIdsStr ? JSON.parse(deletedIdsStr) : [];
        const toSeed = BLOG_POSTS.filter(p => !deletedIds.includes(p.id));
        ...
        for (const post of toSeed) {
          await supabase.from('posts').insert([{
            ...
            service: (post as any).service || 'Médecine Générale',
            image: post.image
          }]);
        }
        await api.settings.set('blog_initialized', 'true');
      } catch (e) {
        console.warn("Exception in ensureInitialized blog:", e);
      }
    },

    getAll: async (): Promise<BlogPost[]> => {
      try {
        const isInitialized = await api.settings.get('blog_initialized');
        const deletedIdsStr = await api.settings.get('deleted_blog_ids');
        const deletedIds: number[] = deletedIdsStr ? JSON.parse(deletedIdsStr) : [];
        ...
        if (error) {
          ...
          if (isInitialized !== 'true') {
            return BLOG_POSTS
              ...
              .map((p, i) => ({ ...p, created_at: new Date().toISOString(), id: p.id || i + 1000 }));
          }
          return [];
        }
        ...
        if (data && data.length > 0) {
          return data
            .filter((item: any) => !deletedIds.includes(Number(item.id)))
            .map((item: any) => ({
              id: Number(item.id),
              title: item.title,
        ...
        // Not initialized yet and table is empty:
        return BLOG_POSTS
          .filter(p => !deletedIds.includes(p.id))
          .map((p, i) => ({ ...p, created_at: new Date().toISOString(), id: p.id || i + 1000 }));
      } catch (e) {
    ...
    getById: async (id: number): Promise<BlogPost | null> => {
      try {
        ...
        if (error || !data) {
          ...
          return fallback ? ({ ...fallback, created_at: new Date().toISOString() } as any) : null;
        }
        return {
          id: Number(data.id),
          title: data.title,
    ...
    delete: async (id: number, imageUrl?: string) => {
      ...
      if (imageUrl) {
        ...
        if (path) {
          try {
            ...
            console.warn("Storage image remove warning:", storageErr);
          }
        }
      }
      ...
      // Track deleted ID so default post or cached post never reappears
      try {
        const deletedIdsStr = await api.settings.get('deleted_blog_ids');
        const deletedIds: number[] = deletedIdsStr ? JSON.parse(deletedIdsStr) : [];
        if (!deletedIds.includes(id)) {
          deletedIds.push(id);
          await api.settings.set('deleted_blog_ids', JSON.stringify(deletedIds));
        }
        await api.settings.set('blog_initialized', 'true');
      } catch (settingErr) {
        console.warn("Erreur mise à jour deleted_blog_ids:", settingErr);
      }
