...
export const AdminMessages: React.FC<AdminMessagesProps> = ({
  ...
  return (
    <div className="space-y-4">
      {/* Detail Modal */}
      {viewingMessage && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in overflow-y-auto">
          <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl max-w-lg w-full max-h-[90vh] flex flex-col p-6 md:p-8 animate-in zoom-in-95 relative border border-gray-100 dark:border-gray-700 overflow-hidden my-auto">
            <button 
              onClick={() => setViewingMessage(null)} 
              className="absolute top-5 right-5 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 z-10"
              aria-label="Fermer"
            >
              <X size={22}/>
            </button>
            <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2 shrink-0">
              <Mail size={24} className="text-teal-600 dark:text-teal-400"/> Message de Contact
            </h3>
            
            <div className="space-y-4 text-sm overflow-y-auto pr-1 min-w-0 flex-1">
              <div className="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-xl border border-gray-100 dark:border-gray-600 min-w-0">
                ...
                <span className="font-bold text-gray-900 dark:text-white text-base break-words block">{viewingMessage.name}</span>
              </div>
              ...
              <div className="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-xl border border-gray-100 dark:border-gray-600 min-w-0">
                <span className="block text-[11px] font-bold text-gray-400 uppercase mb-2">Message</span>
                <div className="max-h-60 overflow-y-auto pr-1">
                  <p className="text-gray-800 dark:text-gray-200 whitespace-pre-wrap break-words [overflow-wrap:anywhere] leading-relaxed">
